...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("PROXYNODE Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  proxynode-cli [options] <command> [params]  " + _("Send command to PROXYNODE Core") + "\n" +
                        "  proxynode-cli [options] help                " + _("List commands") + "\n" +
                        "  proxynode-cli [options] help <command>      " + _("Get help for a command") + "\n";
