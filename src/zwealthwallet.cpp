...
CzPRXWallet::CzPRXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zprx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPRXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPRXSeed_deprecated()) {
                    LogPrintf("%s: Updated zPRX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zprx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zprx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPRXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPRXCount(nCountLastUsed);
    else if (!walletdb.ReadZPRXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPRXWallet::Lock()
{
    seedMaster = 0;
}

void CzPRXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPRXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPRX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPRXWallet::LoadMintPoolFromDB()
{
...
void CzPRXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPRXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPRXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zprxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPRXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPRX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zprxTracker which also adds to database
    pwalletMain->zprxTracker->Add(dMint, true);
...
void CzPRXWallet::SeedToZPRX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPRXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPRXWallet::GenerateDeterministicZPRX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPRXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPRXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
