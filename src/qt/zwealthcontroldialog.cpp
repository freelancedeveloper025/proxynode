...
#include "zprxcontroldialog.h"
#include "ui_zprxcontroldialog.h"
...
std::set<std::string> ZPrxControlDialog::setSelectedMints;
std::set<CMintMeta> ZPrxControlDialog::setMints;

ZPrxControlDialog::ZPrxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZPrxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZPrxControlDialog::~ZPrxControlDialog()
{
    delete ui;
}

void ZPrxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPrxControlDialog::updateList()
{
...
void ZPrxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPrxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZPrx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPrxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZPrxControlDialog::GetSelectedMints()
{
...
void ZPrxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
