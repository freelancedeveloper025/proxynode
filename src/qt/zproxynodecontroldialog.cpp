...
#include "ui_zproxynodecontroldialog.h"
...
std::list<std::string> ZPROXYNODEControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZPROXYNODEControlDialog::listMints;

ZPROXYNODEControlDialog::ZPROXYNODEControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZPROXYNODEControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZPROXYNODEControlDialog::~ZPROXYNODEControlDialog()
{
    delete ui;
}

void ZPROXYNODEControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPROXYNODEControlDialog::updateList()
{
...
void ZPROXYNODEControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPROXYNODEControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZPROXYNODE_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPROXYNODEControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZPROXYNODEControlDialog::GetSelectedMints()
{
...
void ZPROXYNODEControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
