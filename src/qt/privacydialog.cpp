...
#include "zproxynodecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPRX ought to be enough for anybody." - Bill Gates, 2017
    ui->zPRXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPRXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPRX->setEnabled(false);
        ui->pushButtonMintzPRX->setToolTip(tr("zPRX is currently disabled due to maintenance."));

        ui->pushButtonSpendzPRX->setEnabled(false);
        ui->pushButtonSpendzPRX->setToolTip(tr("zPRX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPRXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPRX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPRX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPRX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPRX in ") +
...
void PrivacyDialog::on_pushButtonSpendzPRX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPRX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPRX
        sendzPRX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPRX
    sendzPRX();
}

void PrivacyDialog::on_pushButtonZPROXYNODEControl_clicked()
{
    ZPROXYNODEControlDialog* zPRXControl = new ZPROXYNODEControlDialog(this);
    zPRXControl->setModel(walletModel);
    zPRXControl->exec();
}

void PrivacyDialog::setZPROXYNODEControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzPRXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPRX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid PROXYNODE Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPRXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPRX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PROXYNODE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPRX</b>";
    ...
    // use mints from zPRX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZPROXYNODEControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZPROXYNODEControlDialog::GetSelectedMints();
    }
    ...
    // Clear zproxynode selector in case it was used
    ZPROXYNODEControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zPRX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " PROXYNODE, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zPRX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPRX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPRX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPRX "));
    ui->labelzPRXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzPRXSyncStatus->setVisible(fShow);
}
